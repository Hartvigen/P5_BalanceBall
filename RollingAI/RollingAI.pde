...
void initialize()
{
  ...
  if (data == -1)
  {
    ...
    tables[0] = new Table(250, new PVector(width/2, height/2), new FirstBrain(loadBrain(input, 0)), maxFitness);
      try {
        input.read();
      }
      catch(IOException e) {
    ...
    for (int i = 1; i < genSize; i++) {
      tables[i] = new Table(250, new PVector(width/2, height/2), new FirstBrain(loadBrain(input, i))); //<>//
      try {
        input.read();
      }
      catch(IOException e) {
...
void mutate()
{
  ...
  for (int i = 1; i < genSize - 1; i++)
    newTables[i].brain.mutate(0.3, 0.06);
...
float[][][] loadBrain(InputStream input, int brainCount)
{
  float[][][] weights = new float[2][6][7];
  loadHiddenLayer(input, weights[0], brainCount);
  try {
    input.read();
    input.read();
  }
...
void loadHiddenLayer(InputStream input, float[][] weights, int brainCount)
{
  int data = 0;
